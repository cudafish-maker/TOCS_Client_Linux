...
Network Transports
TOCS uses the Reticulum Network Stack and works over any combination of:
...
Check Python 3.10+ and install pip / venv if needed
Create a virtual environment (venv/) and install Python dependencies
Download and install Yggdrasil (auto-detects amd64 / arm64 / armhf)
Inject public peers into the Yggdrasil config and start the service
Optionally configure I2P (if i2prouter or i2pd is installed)
Optionally configure an RNode LoRa radio (detects serial port, uses 915 MHz defaults)
Write a Reticulum config pre-pointed at the TOCS server
Create a tocs_client.sh launch script
First Launch
bash tocs_client.sh
